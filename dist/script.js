...
function resetGame() {
  ...
  if (hardMode) {
    noteElement.innerHTML = `Нажмите джойстик, чтобы начать!
        <p>Жесткий режим, избегайте черных дыр. Вернуться к простому режиму? Нажмите Е</p>`;
  } else {
    noteElement.innerHTML = `Нажмите джойстик, чтобы начать!
        <p>Жесткий режим, избегайте черных дыр. Вернуться к простому режиму? Нажмите Е</p>`;
  }
...
function main(timestamp) {
  ...
  try {
    ...
    if (
      ...
      noteElement.innerHTML = `Поздравляем, вы сделали это!
        ${!hardMode ? "<p>Нажмите H для жесткого режима</p>" : ""}
        <p>
          Follow me
    ...
    if (error.message == "The ball fell into a hole") {
      noteElement.innerHTML = `Мяч попал в черную дыру! Нажмите пробел, чтобы сбросить игру..
        <p>
          Вернуться к легкому? Нажмите Е
        </p>`;
      noteElement.style.opacity = 1;
      gameInProgress = false;
    } else throw error;
